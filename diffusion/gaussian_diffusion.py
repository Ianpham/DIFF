...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output, attn_channel_masks, mlp_weight_masks, masks = model(x_t, t, **model_kwargs)
            model_kwargs["complete_model"] = True
            complete_model_output, _, _, _= model(x_t, t, **model_kwargs)
            ...
            if self.model_var_type in [
                ...
                #################################### dynamic model ####################################
                model_output, model_var_values = th.split(model_output, C, dim=1) # 网络预测的均值和方差。
                ...
                #################################### complete model ####################################
                complete_model_output, complete_model_var_values = th.split(complete_model_output, C, dim=1)
                frozen_out = th.cat([complete_model_output.detach(), complete_model_var_values], dim=1)
                terms["complete_vb"] = self._vb_terms_bpd(
                    model=lambda *args, r=frozen_out: r,
                    x_start=x_start,
                    x_t=x_t,
                    t=t,
                ...
                if self.loss_type == LossType.RESCALED_MSE:
                    # Divide by 1000 for equivalence with initial implementation.
                    # Without a factor of 1/1000, the VB term hurts the MSE term.
                    terms["complete_vb"] *= self.num_timesteps / 1000.0
            ...
            terms["complete_mse"] = mean_flat((target - complete_model_output) ** 2)
        ...
        return terms, attn_channel_masks, mlp_weight_masks, masks

    def training_losses_without_completemodel(self, model, x_start, t, model_kwargs=None, noise=None):
        """
        Compute training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.
